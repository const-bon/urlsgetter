...
def get_scheme(scheme):
    ...
    # scheme_pat = re.compile('([a-z0-9\+\.\-]+:)?(//)?([^ \s<>\"#\{\}\|/\^~\[\]`]?:?[^ \s<>\"#\{\}\|/\^~\[\]`]?)(@)?[a-z0-9]+.[a-z]+')
    url_pat = scheme_pat+specific_pat+user_pass_pat
    scheme = str(scheme)
    scheme = scheme.lower()
    scheme = re.findall(url_pat, scheme)
    # new_data = [''.join(w) for w in scheme]
    return scheme
...
def urls_getter():
    ...
    # console_print(r.status_code)
    # console_print(r.headers)
    # console_print(r.content)
    # console_print("")
    # urls = GETURL.findall(str(r.content))
    # console_print(urls)
    #urls = FINDURL.findall(content)
    #console_print(urls)
    #urls = re.findall('[a-z]*[:]*[//]*www', str(r.content))
    #console_print(urls)
    # urls = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', url)
    console_print(get_scheme(content))
